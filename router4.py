...
def start_server():
    ...
    while True:
        ...
        try:
            thread1 = Thread(target=processing_thread, args=(connection, ip, port, forwarding_table_with_range, default_gateway_port, (router5, router6)))
            thread1.start()
        except:
...
def processing_thread(connection, ip, port, forwarding_table_with_range, default_gateway_port, routers, max_buffer_size=5120):
    ...
    while True:
        ...
        new_ttl = int(ttl) - 1
        new_packet = '{},{},{},{}'.format(sourceIP, destinationIP, payload, new_ttl)
        ...
        for row in forwarding_table_with_range:
            dest = row[0]
            if destinationIP_int >= dest[0] and destinationIP_int <= dest[1]:
                sending_port = row[3]
                break
        ...
        elif sending_port == '8006' and new_ttl > 0:
